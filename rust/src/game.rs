...
pub fn run_game<const HEIGHT: usize, const WIDTH: usize, const GENERATIONS: u32>() {
    ...
    for _ in 0..GENERATIONS {
        // screen.draw(&mut buf);
        screen.draw_and_next_gen(&mut buf);
...
impl<const WIDTH: usize, const HEIGHT: usize> Screen<WIDTH, HEIGHT> {
    ...
    #[inline]
    // Literally just the draw and next_gen functions interleaved into one. It seems to be faster,
    // somehow!
    //
    // DRY evangelists are sobbing rn
    fn draw_and_next_gen(&mut self, buf: &mut Vec<u8>) {
        #[inline(always)]
        fn lives(cell: bool, adjacent: u8) -> bool {
            (adjacent == 3) | (cell && adjacent == 2)
        }
        ...
        unsafe {
            // First row
            Self::write_line(buf, self.swap.get_unchecked(..WIDTH));
            // Top left cell
            cell! {
                (0, 0),
                [
            ...
            Self::write_line(
                buf,
                self.swap
                    .get_unchecked(bottom_row * WIDTH..)
                    .get_unchecked(..WIDTH),
            );
            // Bottom left cell
            cell! {
